...
class _PlayListScreenState extends State<PlayListScreen> {
  List<GuitarRecording> _recordings = [];
  int? _expandedIdx; // 재생목록에 갤럭시처럼 확장후 재생을 구현을 위해 확장된 목록의 index
  GuitarSound? _soundEngine;// 구현해놓은 기타 사운드 엔진 가져옴
  Timer? _timer; //시간관련 동작을 위해 진행상황을 콜백하는 용도
  double _elapsed = 0.0; //경과시간 시작은0초라서 0초로 초기화
  int _eventIdx = 0;
  ...
  GuitarRecording? get _expandedRec =>
      _expandedIdx == null ? null : _recordings[_expandedIdx!];

  //가져온 녹음본의 총 재생길이를 가져오는 비어있으면 0.0초(취소,확장안됨) 있다면 이벤트의의 마지막리스트
  //는 총길이의 초를 가지고 있다 => 이걸 가져와서 총길이로 한다
  double get _totalDuration => _expandedRec?.events.isEmpty ?? true
      ? 0.0
      : _expandedRec!.events.last.time;
  ...
  void _deleteRecording(int idx) async {
    await deleteRecording(_recordings[idx].id);
    //guiarRecord에있는 삭제함수발생 및 로드(setstate)
    _loadRecordings();
  }

  //실행할 녹음본 클릭했을때 불러올 것들
  void _expandTile(int idx) {
    if (_expandedIdx == idx) return; // 이미 열려있으면 무시
    setState(() {
      _expandedIdx = idx;             //확장인덱스
      _soundEngine?.dispose();        //기존 사운드엔진있으면 종로
      _soundEngine = GuitarSound();   //사운드엔진 생성
      _soundEngine!.load();           //사운드엔진 샘플초기화(0~6번줄)
      _timer?.cancel();               //재생중이던 타이머있으면 정지
      _elapsed = 0.0;                 //재생시간0초초기화
      _eventIdx = 0;                  //이벤트인덱스 0초로 초기화
      _isPlaying = false;             //재생중x로 초기화 재생버튼을 눌러야 실행
    });
  }
  ...
  void _play() {
    ...
    if (_expandedRec == null || _isPlaying) return;
    ...
    _timer = Timer.periodic(Duration(milliseconds: 20), (timer) {
      ...
      while (_eventIdx < _expandedRec!.events.length &&
          _expandedRec!.events[_eventIdx].time <= _elapsed) {
        final ev = _expandedRec!.events[_eventIdx];
        //재생해야할 이벤트를 가져오고
        //playNote로 이벤트에 저장된 sting, fret실행
        _soundEngine!.playNote(ev.stringNum, ev.fretNum);
        _eventIdx++;
        //인덱스 증가
      }
      //총길이보다 넘는다면 자동으로 멈추게
      setState(() {});
      if (_elapsed >= _totalDuration) _stop();
    });
  }
  ...
  void _seek(double v) {
    setState(() {
      _elapsed = v;
      //이벤트인덱스위치를 찾고
      _eventIdx = _expandedRec!.events.indexWhere((e) => e.time >= v);
      //이벤트가없다면(끝이라면) 이벤트길이만큼 맞춰준다
      if (_eventIdx < 0) _eventIdx = _expandedRec!.events.length;
    });
  }
                ...
                trailing: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () => _deleteRecording(idx),
                    ),
                  ],
                ),
                //원하는 녹음본 탭하면 그 인덱스에맞는 저거 불러와짐 listtitle의ontap
                onTap: () => _expandTile(idx),
              ),
                ...
                child: isExpanded//확장이되면
                  ...
                  children: [
                    Slider(
                      ...
                      min: 0,
                      max: _totalDuration,
                      value: _elapsed.clamp(0, _totalDuration),
                      onChanged: _seek,//슬라이더전용함수 위에만들어둠
                    ),
                    Row(
                      //재생 시간 현재초
                      children: [
                        Text(
                          '${_elapsed.toStringAsFixed(2)} / ${_totalDuration.toStringAsFixed(2)}초',
